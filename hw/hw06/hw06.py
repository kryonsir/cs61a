...
class VendingMachine:
    ...
    def add_funds(self,num):
        if self.stock == 0:
            return 'Inventory empty. Restocking required. Here is your ${}.'.format(num)
        self.funds += num
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        temp = Mint.current_year - self.year
        if temp > 50:
...
def is_bst(t):
    ...
    def bst_max(tr):
        if tr.is_leaf():
            ...
            return max([bst_max(b) for b in tr.branches]+[tr.label])

    def bst_min(tr):
        if tr.is_leaf():
            return tr.label
        return min([bst_min(b) for b in tr.branches]+[tr.label])

    if len(t.branches) == 0 or len(t.branches) == 1:
        return True
    elif len(t.branches) == 2:
        if bst_max(t.branches[0]) <= t.label and bst_min(t.branches[1]) > t.label:
            return is_bst(t.branches[0]) & is_bst(t.branches[1])
        else:
            return False
    else:
        return False
...
def path_yielder(t, value):
    ...
    for b in t.branches:
        ...
        for bb in path_yielder(b,value):
            yield res + bb
