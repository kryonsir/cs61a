...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert lower_bound(y)>0 or upper_bound(y)<0, "divided interval spans zero"
...
def quadratic(x, a, b, c):
    ...
    def f(t):
        return a*t*t+b*t+c
    ext_point = -b/(2*a)
    if ext_point < upper_bound(x) and ext_point > lower_bound(x):
        lower = min(f(lower_bound(x)),f(upper_bound(x)),f(ext_point))
        upper = max(f(lower_bound(x)),f(upper_bound(x)),f(ext_point))
    else:
