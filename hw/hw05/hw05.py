...
def make_withdraw(balance, password):
    ...
    errorlist = []
    def func(amount,pwd):
        nonlocal errorlist,balance,password
        if len(errorlist) >= 3:
            return f"Too many incorrect attempts. Attempts: {errorlist}"
        else:
            if pwd != password:
                errorlist.append(pwd)
...
def merge(incr_a, incr_b):
    ...
    while True:
        if next_a == None and next_b == None:
            ...
            yield next_b
            next_b = next(iter_b,v)
        elif next_a != None and next_b == None:
            yield next_a
            next_a = next(iter_a,v)
        else:
            if next_a == next_b:
                yield next_a
                next_a = next(iter_a,v)
