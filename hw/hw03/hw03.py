...
def missing_digits(n):
    ...
    if n < 10: 
        return 0
    temp = n%10 - (n//10)%10 -1 if n%10 > (n//10)%10 else 0
    return missing_digits(n//10) + temp
...
def count_change(total):
    ...
    "*** YOUR CODE HERE ***"
    """
    test fail beacuse below code using while or for
    i,temp = 0 ,total
    while temp >= 2:
        temp //= 2
        i += 1
    """
    # 使用下面这行代码就可以轻松求出不大于total的最大2的次幂
    i = len(bin(total)) - 3
    def f(n,x):
        if n<0:
            return 0
