...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        tattack = other_card.attack
        tdefense = other_card.defense
        player.deck = Deck([Card('card',card.attack + tattack,card.defense + tdefense) for card in player.deck.cards])

        for card in opponent.deck.cards.copy():
            if card.attack == tattack or card.defense == tdefense:
