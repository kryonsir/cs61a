...
def add_trees(t1, t2):
    ...
    tmp = copy_tree(t1) if len1 >= len2 else copy_tree(t2)

    return tree(label(t1)+label(t2),[add_trees(x,y) for x,y in zip(branches(t1),branches(t2))] + branches(tmp)[len_min:]) 
