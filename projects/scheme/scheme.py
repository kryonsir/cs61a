...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if key != nil:
            while key.rest != nil:
                frame.define(key.first,value.first)
                key,value = key.rest, value.rest
            frame.define(key.first,value.first)
        return frame
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        expr = Pair(expressions.first,expressions.rest)
        expr.first = expr.first.rest
...
def do_and_form(expressions, env):
    ...
    while temp is not nil:
        value = scheme_eval(temp.first,env)
...
def make_let_frame(bindings, env):
    ...
    while temp is not nil:
        ...
        name, value = temp.first.first, scheme_eval(temp.first.rest.first,env)
