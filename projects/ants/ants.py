...
class Ant(Insect):
    ...
    def action(self,gamestate):
        if isinstance(self,WallAnt) or isinstance(self,FireAnt):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while temp != None:
            if temp == beehive:
                return None
            if temp.bees and count >= self.min_range and count <= self.max_range:
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.armor <= 0:
            self.palce.ant = self.contained_ant
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        "*** YOUR CODE HERE ***"
        if hasattr(self,"direction") and self.direction == "backwards":
...
def make_slow(action, bee):
    ...
    def new(gamestate):
        nonlocal count
        if gamestate.time%2 == 1 and count < 3:
            count += 1
            return 
        else:
            count += 1
